...
class Message(object):
    ...
    def apply_shift(self, shift):
        ...
        for ch in self.message_text:
            if ch in self.build_shift_dict(shift):
                cipher += self.build_shift_dict(shift)[ch]
            elif ch not in string.ascii_lowercase and ch not in string.ascii_uppercase:
                cipher += ch
        ...
        for i in range(26):
            ...
            for word in words:
                if is_word(word_list, word):
                    ...
                    if len(shifted_words) >= best_shift_num_of_words:
                        best_shift_value = shift
                        best_shift_encrypted_message = test_shift
        
        return (best_shift_value, best_shift_encrypted_message)
